...
int g_frame;
SDL_Texture* g_MooseTexture;
SDL_bool g_done = SDL_FALSE;
...
void loop()
{
    ...
    g_frame = (g_frame + 1) % MOOSEFRAMES_COUNT;
    UpdateTexture(g_MooseTexture, g_frame);

    SDL_RenderClear(g_renderer);
    SDL_RenderCopy(g_renderer, g_MooseTexture, NULL, NULL);
...
int main(int argc, char** argv)
{
    ...
    /* Create the window and g_renderer */
    ...
    if (!window) {
        SDL_LogError(
          SDL_LOG_CATEGORY_APPLICATION, "Couldn't set create window: %s\n", SDL_GetError());
        quit(3);
    }
    ...
    if (!g_renderer) {
        SDL_LogError(
          SDL_LOG_CATEGORY_APPLICATION, "Couldn't set create g_renderer: %s\n", SDL_GetError());
        quit(4);
    }

    g_MooseTexture = SDL_CreateTexture(
      g_renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, MOOSEPIC_W, MOOSEPIC_H);
    if (!g_MooseTexture) {
        SDL_LogError(
          SDL_LOG_CATEGORY_APPLICATION, "Couldn't set create texture: %s\n", SDL_GetError());
        quit(5);
    }
